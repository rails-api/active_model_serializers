...
h4. Embedding Associated Objects

You can also choose to embed associated objects or their ids. You can set this as a preference on your serializer
or override it per association.

<pre lang="ruby">
class PostSerializer < ActiveModel::Serializer
  embed :objects # default
  has_many :comments
end
</pre>
...
You can also embed ids only if you want.
...
You may also you a combination of ids and objects. 
...
*NOTE*: The <code>:embed</code> option defaults to whatever the <code>embed</code> method is. This means, that all
associations will be embedded based on what <code>embed</code> is set to at declaration time. Here is 
an example

<pre lang="ruby">
# embed defaults to :objects
has_many :comments # would serialize as objects
has_many :tags, :embed => :ids # override embed and use ids

embed :ids

has_many :authors # would serialize ids
has_many :other_things, :embed => :objects # overide the id default now
</pre>

This means you could declare your object and id associations like this:
...
has_many :tags
has_many :links
has_many :track_backs
</pre>

You of course can explictly set <code>:embed</code> for any association.
