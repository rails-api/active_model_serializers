...
*NOTE*: The `:embed` option defaults to whatever the `embed` method is. This means, that all
associations will be embedded based on what `embed` is set to at declaration time. Here is 
an example
...
You of course can explictly set `:embed` for any association.
