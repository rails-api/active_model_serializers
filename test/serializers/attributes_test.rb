  ...
  class Serializer
    class AttributesTest < Minitest::Test
      def setup
        ...
        @comment = Comment.new(id: 1, body: 'ZOMG!!', original_format: 'markdown', date: '2015')
      ...
      def test_attributes_inheritance_definition
        assert_equal([:id, :body, :original_format], @serializer_klass._attributes)
      end

      def test_attributes_inheritance
        serializer = @serializer_klass.new(@comment)
        assert_equal({ id: 1, body: 'ZOMG!!', original_format: 'markdown' },
          serializer.attributes)
      end

      def test_attribute_inheritance_with_new_attribute_definition
        assert_equal([:id, :body, :original_format, :date, :likes], @serializer_klass_with_new_attributes._attributes)
        assert_equal([:id, :body, :original_format], CommentSerializer._attributes)
      end

      def test_attribute_inheritance_with_new_attribute
        serializer = @serializer_klass_with_new_attributes.new(@comment)
        assert_equal({ id: 1, body: 'ZOMG!!', original_format: 'markdown', date: '2015', likes: nil },
          serializer.attributes)
      end
